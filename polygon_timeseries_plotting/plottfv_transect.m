...
for var = plot_array
    ...
    for tim = 1:length(def.pdates)
        ...
        for mod = 1:length(ncfile)
            ...
            plot(data(mod).dist,data(mod).pred_lim_ts(3,:),'color',def.median_line(mod).value,'linewidth',0.5,'DisplayName',[ncfile(mod).legend,' (mean-cell)']);hold on
